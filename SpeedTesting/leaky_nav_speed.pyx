...
def boundary_estimate(frame, l_bound, u_bound, l_sat, u_sat, l_val, u_val, dh):
    # convert to HSV
    hist_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    hist = cv2.calcHist([hist_frame], [0], None, [180], [0, 180])
    #print(hist)
...
def init_tracking_mask(xmap, ymap, l_red, u_red, col_frame, wide_mask):    
    ...
    # home_frame now half the size (downsampled)
    _, cnts, _ = cv2.findContours(home_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cnts_lg = [c for c in cnts if cv2.contourArea(c)>150]
    xmin = 720
    xmax = 0
    for c in cnts_lg:
...
def run_tracking_mask(np.ndarray xmap, np.ndarray ymap, np.ndarray l_red, np.ndarray u_red, np.ndarray col_frame, np.ndarray wide_mask, int owidth):
    ...
    # Downsampling improves speed but may compromise feature matching
    gray_frame = cv2.cvtColor(col_frame, cv2.COLOR_BGR2GRAY)
    unwrap_gray = unwarp(gray_frame, xmap, ymap)
    ...
    # downsample frames to speed things up:
    _, cnts, _ = cv2.findContours(home_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def keypoint_height_calc(IK, a0, a2, a3, a4, xS): 
    # should be able to parallelize this ? not really necessary
    for kp_it in IK.keypoints:
        ...
        vc =  vc_w*np.cos(uc_w)
        #print(uc,vc)
        rho = np.sqrt(uc*uc + vc*vc)
        frho = a0 + a2*(rho*rho) + a3*np.power(rho,3) + a4*np.power(rho,4)
        lambda_est = xS/(uc)
...
def est_egomotion(sift_matches, InitialKeypoints, kp_comp_sift):    
    # for some reason, faster when using surf results?
    ...
    # there is probably a way to speed this up but still << than detect and compute features
    # so not relevant for now
...
def omni_balance(cp, np.ndarray omni_frame, np.ndarray mask, np.ndarray l_green, np.ndarray u_green):
    ...
    #cdef vector<vector<Point> > cnts
    #cdef vector<vector<Point> > cnts_lg
    #cdef vector<vector<Point> > cnts_lg
    ...
    if len(cnts)>0:
        ...
        return heading_angle, bal_frame
...
def omni_deposit(np.ndarray cp, np.ndarray omni_frame, np.ndarray mask, np.ndarray l_green, np.ndarray u_green, np.ndarray xmap, np.ndarray ymap):
    ...
    if len(cnts_lg) > 0: 
        ...
        return 0, 0, np.array([0,0]), 0, dep_frame_unwarp

def omni_home(np.ndarray cp, np.ndarray omni_frame, np.ndarray mask, np.ndarray l_red, np.ndarray u_red):
    ...
    if len(cnts) > 0:
        ...
        if len(cnts_lg) > 0:
            return len(cnts_lg), back_frame

        else: return 0,back_frame

    else: return 0,back_frame

def leaving_home(np.ndarray cp, np.ndarray omni_frame, np.ndarray wide_mask, np.ndarray l_red, np.ndarray u_red):
    ...
    return len(cnts_lg), leave_frame
