...
l_red, u_red = boundary_estimate(unwrap_base, lr_bound, ur_bound, 80, 255, 40, 230, 15)
...
for file in files("."):
    ...
    if (not (des_comp_surf is None)) :
        ...
        match_image = cv2.drawMatches(unwrap_base, kp_surf, comp_unwrap, kp_comp_surf, surf_matches[:15], None, flags=2)
        Alist = []
        Blist = []
        for surfdat in surf_matches[:15]:
            ...
            new_uc_w = (kp_comp_surf[surfdat.trainIdx].pt[0])*np.pi/(4*180)
            new_vc_w = (kp_comp_surf[surfdat.trainIdx].pt[1])*300/360
